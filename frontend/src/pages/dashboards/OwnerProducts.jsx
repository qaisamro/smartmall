...
const emptyForm = {
    name_ar: '', price: '', category_id: '', section_id: '', mall_section_id: '', barcode: '',
    description_ar: '', shelf_location: '', link_photo: '', stock_quantity: '', min_stock_alert: '5',
    unit_type: '', pricing_type: '', weight: '', cut: '', preparation: '', notes: ''
};
...
const OwnerProducts = () => {
    ...
    const openEdit = (prod) => {
        ...
        setForm({
            ...
            cut: prod.options?.cut || '',
            preparation: prod.options?.preparation || '',
            notes: prod.options?.notes || ''
        });
    ...
    const handleSubmit = (e) => {
        ...
        const metadata = {
            ...
            weight: form.weight,
            options: { cut: form.cut.trim(), preparation: form.preparation.trim(), notes: form.notes.trim() }
        };
        if (Object.values(metadata).some(value => typeof value === 'object' ? Object.values(value).some(Boolean) : Boolean(value))) {
            payload.unit_type = metadata.unit_type || null;
            payload.pricing_type = metadata.pricing_type || null;
            payload.weight = metadata.weight === '' ? null : Number(metadata.weight);
            payload.options = Object.fromEntries(Object.entries(metadata.options).filter(([, value]) => value));
        }
                            ...
                            {scanMode ? (
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4 text-right">
                                    ...
                                    <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-4 space-y-4">
                                        <div>
                                            <h4 className="text-sm font-bold text-emerald-300">بيانات البيع الاختيارية</h4>
                                            <p className="text-[11px] text-gray-500 mt-1">اترك هذه الحقول فارغة للمنتجات الحالية أو ذات السعر الثابت.</p>
                                        </div>
                                        <div className="grid sm:grid-cols-3 gap-3">
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">الوحدة</label>
                                                <input type="text" value={form.unit_type} onChange={e => setForm({ ...form, unit_type: e.target.value })} className="input-field text-right" placeholder="كغم، قطعة..." />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">نوع التسعير</label>
                                                <select value={form.pricing_type} onChange={e => setForm({ ...form, pricing_type: e.target.value })} className="input-field text-right">
                                                    <option value="">بدون تحديد</option>
                                                    <option value="fixed">سعر ثابت</option>
                                                    <option value="per_unit">حسب الوحدة</option>
                                                    <option value="per_weight">حسب الوزن</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">الوزن</label>
                                                <input type="number" min="0" step="0.001" value={form.weight} onChange={e => setForm({ ...form, weight: e.target.value })} className="input-field text-right font-mono" placeholder="اختياري" />
                                            </div>
                                        </div>
                                        <div className="grid sm:grid-cols-3 gap-3">
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">نوع القطعة</label>
                                                <input type="text" value={form.cut} onChange={e => setForm({ ...form, cut: e.target.value })} className="input-field text-right" placeholder="شرائح، مفروم..." />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">التحضير</label>
                                                <input type="text" value={form.preparation} onChange={e => setForm({ ...form, preparation: e.target.value })} className="input-field text-right" placeholder="منزوع العظم..." />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-400 mb-1.5 ml-2">ملاحظات</label>
                                                <input type="text" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} className="input-field text-right" placeholder="اختياري" />
                                            </div>
                                        </div>
                                    </div>
