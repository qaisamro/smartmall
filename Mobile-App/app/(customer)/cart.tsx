...
export default function CartScreen() {
  ...
  const storeGroups = useMemo<CartStoreGroup[]>(() => {
    const groups = new Map<number, CartStoreGroup>();

    for (const item of items) {
      const existing = groups.get(item.mallId);
      if (existing) {
      ...
      const mall = malls?.find((candidate) => candidate.id === item.mallId);
      groups.set(item.mallId, {
        mallId: item.mallId,
        name: mall ? (isAr ? mall.name_ar : mall.name_en || mall.name_ar) : t('cart.store_fallback', { id: item.mallId }),
        items: [item],
  ...
  }, [i18n.language, isAr, items, malls, t]);
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <FlatList
        ...
        renderItem={({ item: row }) => {
          const { item, group, isFirstInGroup } = row;
          return (
            <View style={styles.rowGroup}>
              {isFirstInGroup ? (
                <View style={[styles.storeHeader, { backgroundColor: colors.primarySoft, borderColor: colors.border }]}>
                  <View style={[styles.storeHeaderIcon, { backgroundColor: colors.primary }]}>
                    <Feather name="shopping-bag" size={16} color={colors.primaryForeground} />
                  </View>
                  <View style={styles.storeHeaderCopy}>
                    <Text style={[styles.storeHeaderLabel, { color: colors.mutedForeground }]}>{t('cart.store')}</Text>
                    <Text style={[styles.storeHeaderName, { color: colors.foreground }]} numberOfLines={1}>
                      {group.name}
                    </Text>
                  </View>
                  <View style={styles.storeHeaderTotals}>
                    <Text style={[styles.storeHeaderItems, { color: colors.mutedForeground }]}>
                      {t('cart.store_items', { count: formatNumber(group.itemCount) })}
                    </Text>
                    <Text style={[styles.storeHeaderSubtotal, { color: colors.primary }]}>
        ...
        ListFooterComponent={
          <View style={styles.footer}>
            ...
            {storeGroups.length > 1 ? (
              <View style={[styles.storeSummary, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
                <Text style={[styles.storeSummaryTitle, { color: colors.foreground }]}>{t('cart.store_breakdown')}</Text>
                {storeGroups.map((group) => (
                  <View key={group.mallId} style={styles.storeSummaryRow}>
                    <Text style={[styles.storeSummaryName, { color: colors.mutedForeground }]} numberOfLines={1}>
                      {group.name}
                    </Text>
                    <Text style={[styles.storeSummaryValue, { color: colors.foreground }]}>
                      {formatCurrency(group.subtotal)}
                    </Text>
                  </View>
                ))}
...
const styles = StyleSheet.create({
  ...
  storeHeaderCopy: { flex: 1, minWidth: 0, gap: 2 },
  storeHeaderLabel: { fontFamily: 'Inter_500Medium', fontSize: 10 },
  storeHeaderName: { fontFamily: 'Inter_700Bold', fontSize: 14 },
  storeHeaderTotals: { alignItems: 'flex-end', gap: 2 },
  storeHeaderItems: { fontFamily: 'Inter_500Medium', fontSize: 10 },
  storeHeaderSubtotal: { fontFamily: 'Inter_700Bold', fontSize: 13 },
  titleShell: {
    flex: 1,
    minHeight: 42,
    marginHorizontal: 12,
  ...
  storeSummary: { borderWidth: 1, borderRadius: 15, padding: 14, gap: 10 },
  storeSummaryTitle: { fontFamily: 'Inter_700Bold', fontSize: 13 },
  storeSummaryRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
  storeSummaryName: { flex: 1, fontFamily: 'Inter_500Medium', fontSize: 12 },
  storeSummaryValue: { fontFamily: 'Inter_700Bold', fontSize: 12 },
