...
import type { ProductOptions, ProductUnit, PricingType } from '@/src/types/storeTypes';
...
function sameCartLine(left: CartItem, right: CartItem) {
  if (left.productId !== right.productId || left.mallId !== right.mallId) return false;
  if (left.unitType !== right.unitType || left.pricingType !== right.pricingType) return false;
...
export const useCartStore = create<CartState>()(
  persist(
    (set, get) => ({
      ...
      addItem: (item) =>
        set((state) => {
          const existing = state.items.find((entry) => sameCartLine(entry, item));
          return existing
            ? {
