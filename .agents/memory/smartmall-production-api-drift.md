...
The live API may also return additive catalog metadata as an encoded legacy string while still exposing the old enum value; mobile presentation code should decode that format as a compatibility fallback and continue supporting normalized API fields.

**Why:** Test catalog rows were inserted successfully in production, but the deployed API returned `supermarket` and raw metadata strings because the repository model accessors had not been deployed.

**How to apply:** Resolve store type and product presentation metadata from both normalized fields and the encoded prefixes before treating the production API as incomplete.