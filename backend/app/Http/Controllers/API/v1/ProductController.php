...
{
    private const PRODUCT_METADATA_RULES = [
        'unit_type' => 'sometimes|nullable|string|max:50',
        'pricing_type' => 'sometimes|nullable|in:fixed,per_unit,per_weight',
        'weight' => 'sometimes|nullable|numeric|min:0|max:1000000',
        'options' => 'sometimes|nullable|array',
        'options.cut' => 'sometimes|nullable|string|max:100',
        'options.preparation' => 'sometimes|nullable|string|max:100',
        'options.notes' => 'sometimes|nullable|string|max:240',
        'cut' => 'sometimes|nullable|string|max:100',
        'preparation' => 'sometimes|nullable|string|max:100',
    ];
    ...
    private function metadataFromRequest(Request $request): array
    {
        ...
        $metadata = [];
        foreach (['unit_type', 'pricing_type', 'weight'] as $key) {
            if ($request->filled($key)) {
                $metadata[$key] = $key === 'weight' ? (float) $request->input($key) : trim((string) $request->input($key));
            }
        }

        $approvedOptions = is_array($options) ? array_intersect_key($options, array_flip(['cut', 'preparation', 'notes'])) : [];
        foreach (['cut', 'preparation'] as $key) {
            if ($request->filled($key)) {
                $approvedOptions[$key] = trim((string) $request->input($key));
            }
        }
        $approvedOptions = array_filter($approvedOptions, static fn ($value) => $value !== null && $value !== '');
        if ($approvedOptions !== []) {
            $metadata['options'] = $approvedOptions;
        }

        return $metadata;
    }

    private function hasMetadataInput(Request $request): bool
    {
        return $request->hasAny(['unit_type', 'pricing_type', 'weight', 'options', 'cut', 'preparation']);
    }
        ...
        return response()->json(
            Product::with(['shelves:id', 'category:id,name_ar,name_en', 'section:id,name_ar,name_en', 'mallSection:id,name_ar,name_en', 'barcodeOverride'])
                ->select('id', 'mall_id', 'category_id', 'section_id', 'mall_section_id', 'name_ar', 'name_en', 'description_ar', 'description_en',
                    'price', 'discount_price', 'barcode', 'sku', 'brand', 'unit', 'image', 'link_photo', 'is_active', 'hide_stock_from_customer')
                ->findOrFail($id)
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($this->hasMetadataInput($request)) {
            $data['description_en'] = Product::encodeDescriptionMetadata(
                $request->input('description_en'),
                $this->metadataFromRequest($request)
            );
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate(self::PRODUCT_METADATA_RULES);
        ...
        if ($this->hasMetadataInput($request)) {
            $updates['description_en'] = Product::encodeDescriptionMetadata(
                $request->input('description_en', $product->description_en),
                $this->metadataFromRequest($request)
            );
        }
    ...
    public function adminLookupByBarcode(Request $request)
    {
        ...
        return response()->json(['found'=>true,'data'=>$product->only(['id','name_ar','name_en','price','barcode','category_id','section_id','link_photo','unit','unit_type','pricing_type','weight','options'])]);
    }
    ...
    public function adminStore(Request $request) {
        ...
        $data = $request->only(['mall_id','category_id','section_id','mall_section_id','name_ar','name_en','price','link_photo','shelf_location','description_en']);
        if ($this->hasMetadataInput($request)) {
            $data['description_en'] = Product::encodeDescriptionMetadata(
                $request->input('description_en'),
                $this->metadataFromRequest($request)
    ...
    public function adminUpdate(Request $request,$id){
        ...
        $updates = $request->only(['name_ar','name_en','price','barcode','link_photo','stock_quantity','shelf_location','description_en']);
        if ($this->hasMetadataInput($request)) {
            $updates['description_en'] = Product::encodeDescriptionMetadata(
                $request->input('description_en', $p->description_en),
                $this->metadataFromRequest($request)
            );
        }
        $p->update($updates);
