...
export default function CheckoutScreen() {
  ...
  const filteredDeliveryZones = useMemo(() => {
    const query = deliveryZoneSearch.trim().toLocaleLowerCase();
    if (!query) return deliveryZonesQuery.data ?? [];
    return (deliveryZonesQuery.data ?? []).filter((zone) => zone.name.toLocaleLowerCase().includes(query));
  }, [deliveryZoneSearch, deliveryZonesQuery.data]);
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView
        ...
        contentContainerStyle={[styles.content, { paddingTop: insets.top + 60, paddingBottom: insets.bottom + 180 }]}
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.titleShell}>
            <Text style={[styles.title, { color: colors.primary }]}>{t('checkout.title') || 'إتمام الطلب'}</Text>
          </View>
      ...
      {deliveryMethod === 'delivery' && deliveryZonesQuery.data?.length ? (
        <>
          ...
          <View style={[styles.zoneCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={[styles.zoneSearch, { backgroundColor: colors.background, borderColor: colors.border }]}>
              <Feather name="search" size={17} color={colors.mutedForeground} />
              <TextInput
                accessibilityLabel={t('checkout.delivery_zone_search')}
                value={deliveryZoneSearch}
                onChangeText={setDeliveryZoneSearch}
                placeholder={t('checkout.delivery_zone_search')}
                placeholderTextColor={colors.mutedForeground}
                style={[styles.zoneSearchInput, { color: colors.foreground, textAlign: 'right' }]}
                autoCapitalize="none"
                returnKeyType="search"
              />
              {deliveryZoneSearch.length > 0 ? (
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={t('common.clear')}
                  onPress={() => setDeliveryZoneSearch('')}
            ...
            {filteredDeliveryZones.length === 0 ? (
              <Text style={[styles.noZones, { color: colors.mutedForeground }]}>
                {t('checkout.delivery_zone_empty')}
              </Text>
            ) : null}
            </ScrollView>
          </View>
      ...
      <View
        style={[
          styles.actionFooter,
          {
            ...
            bottom: Math.max(insets.bottom, 9) + 72,
          },
        ]}
      >
        <Pressable
          ...
          style={({ pressed }) => [
            styles.submitButton,
            {
              ...
              opacity: isSubmitting || items.length === 0 ? 0.5 : pressed ? 0.82 : 1,
            },
          ]}
        >
          {isSubmitting ? (
            ...
            <>
              <Text style={[styles.submitButtonText, { color: colors.primaryForeground }]}>
                {deliveryMethod === 'in-mall' ? t('checkout.continue_to_scan') : t('checkout.place_order')}
              </Text>
              <Feather
                name={deliveryMethod === 'in-mall' ? 'maximize' : 'check'}
                size={18}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontFamily: 'Inter_700Bold', fontSize: 13, marginTop: 2, textAlign: 'right' },
  notice: {
    borderWidth: 1,
    borderRadius: 17,
    padding: 14,
  ...
  noticeText: { flex: 1, fontFamily: 'Inter_500Medium', fontSize: 12, lineHeight: 20, textAlign: 'right' },
  methodList: { gap: 10 },
  methodOption: {
    minHeight: 55,
    borderWidth: 1,
  ...
  zoneCard: { borderWidth: 1, borderRadius: 17, padding: 10, gap: 10 },
  zoneSearch: {
    minHeight: 46,
    borderWidth: 1,
    borderRadius: 14,
  ...
  zoneSearchInput: { flex: 1, height: '100%', fontFamily: 'Inter_400Regular', fontSize: 13 },
  clearSearch: { padding: 3 },
  zoneList: { maxHeight: 242 },
  zoneListContent: { gap: 8, paddingBottom: 2 },
  noZones: { paddingVertical: 18, textAlign: 'center', fontFamily: 'Inter_500Medium', fontSize: 13 },
  zoneFee: { fontFamily: 'Inter_600SemiBold', fontSize: 12, marginStart: 'auto' },
  multiline: { minHeight: 82, height: undefined, paddingTop: 14, textAlignVertical: 'top', textAlign: 'right' },
  summary: {
    borderWidth: 1,
    borderRadius: 17,
    padding: 16,
  ...
  itemText: { flex: 1, fontFamily: 'Inter_500Medium', fontSize: 12, textAlign: 'right' },
  ...
  total: { fontFamily: 'Inter_800ExtraBold', fontSize: 19 },
  ...
  submitButtonText: { fontFamily: 'Inter_700Bold', fontSize: 14, letterSpacing: 0.2 },
});